void polydl_lib_matmul_f32_fma( long long int M, long long int N, long long int K,long long int A_stride, long long int B_stride, long long int C_stride, float *A, float *B, float *C) {
	__m512 vec_C;
	__m512 vec_B, vec_B_1, vec_B_2, vec_B_3;
	__m512 vec_A, vec_A_1, vec_A_2, vec_A_3;
	int i, j, k;
	int i_aux, j_aux;
	...
	for (i = 0; i < M_full; i += 16) {
		for (j = 0; j < N_full; j += 1) {
			...
			for (k = 0; k < K_full; k += 4) {
				...
				vec_B_2 = _mm512_set1_ps(B[(k+2)*B_stride + j]);
				vec_B_3 = _mm512_set1_ps(B[(k+3)*B_stride + j]);

				float A_aux[16], A_aux_1[16], A_aux_2[16], A_aux_3[16];
				for (int i2 = 0; i2 < 16; i2++) {
					A_aux[i2] = A[(i+i2)*A_stride+k];
					A_aux_1[i2] = A[(i+i2)*A_stride+k+1];
					A_aux_2[i2] = A[(i+i2)*A_stride+k+2];
					A_aux_3[i2] = A[(i+i2)*A_stride+k+3];
				}

				vec_A = _mm512_load_ps((__m512*)A_aux);
				vec_A_1 = _mm512_load_ps((__m512*)A_aux_1);
				vec_A_2 = _mm512_load_ps((__m512*)A_aux_2);
				vec_A_3 = _mm512_load_ps((__m512*)A_aux_3);

				vec_C = _mm512_fmadd_ps(vec_A, vec_B, vec_C);
				vec_C = _mm512_fmadd_ps(vec_A_1, vec_B_1, vec_C);
				vec_C = _mm512_fmadd_ps(vec_A_2, vec_B_2, vec_C);
				vec_C = _mm512_fmadd_ps(vec_A_3, vec_B_3, vec_C);
			}
