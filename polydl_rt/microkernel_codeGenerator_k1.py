...
def declaration(step_M,step_N,step_K):
    ...
    if step_M > 1 : 
        for variable in ['Ai','acci']:
            f.write('__m512 ')
            for i in range (0,(step_M-1)*int(step_K/16)):
                f.write('vec_' + variable +str(i))
                if( i != (step_M-1)*int(step_K/16) - 1) :
                    f.write(', ')
            f.write(';\n')
...
def loopOver(step_M,step_N,step_K):
    ...
    if step_M > 1 : 
        for j in range(0, (step_M-1)):
            for i in range(0, int(step_K/16)):
                f.write('vec_acci%d = _mm512_setzero_ps();\n' % (j*int(step_K/16) + i))
    ...
    # (step_M-1)*int(step_K/16)
    if step_M > 1 : 
        for j in range(0, (step_M-1)):
            for i in range(0, int(step_K/16)):
                f.write('vec_Ai%d = _mm512_load_ps((__m512*)&A[(i+%d)*A_stride + (k+%d)]);\n'%((j*int(step_K/16))+i,j+1,i*16))
                f.write('vec_acci%d = _mm512_fmadd_ps(vec_Ai%d, vec_B%d,vec_acci%d);\n'%((j*int(step_K/16))+i,(j*int(step_K/16))+i,i,(j*int(step_K/16))+i))
    ...
    if step_M > 1 : 
        for j in range(0, (step_M-1)):
            for i in range(0, int(step_K/16)):
                # f.write('vec_Ai%d = _mm512_load_ps((__m512*)&A[(i+%d)*A_stride + (k+%d)]);\n'%((j*int(step_K/16))+i,j+1,i*16))
                # f.write('vec_acci%d = _mm512_fmadd_ps(vec_Ai%d, vec_B%d,vec_acci%d);\n'%((j*int(step_K/16))+i,(j*int(step_K/16))+i,i,(j*int(step_K/16))+i))
                f.write('C[(i+%d)*C_stride + j] += _mm512_reduce_add_ps (vec_acci%d);\n'%(j+1,(j*int(step_K/16))+i))
