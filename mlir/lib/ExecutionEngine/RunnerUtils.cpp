...
extern "C" void print_f32_polydl(long long int rank, long long int offset,
...
_mlir_ciface_print_memref_f32_polydl(UnrankedMemRefType<float> *M) {
  ...
  std::cout << "base@ = " << reinterpret_cast<void *>(V.data)
  ...
  print_f32_polydl(V.rank, V.offset, V.sizes[0], V.sizes[1], V.strides[0],
                   V.strides[1], reinterpret_cast<void *>(V.data));
}

extern "C" void polydl_lib_matmul_f32(long long int M, long long int N,
                                      long long int K, long long int A_stride,
                                      long long int B_stride,
                                      long long int C_stride, float *A,
                                      float *B, float *C);

extern "C" void polydl_matmul_f32(int64_t A_rank, void *A_ptr, int64_t B_rank,
                                  void *B_ptr, int64_t C_rank, void *C_ptr,
  ...
  if (A.rank != 2 || B.rank != 2 || C.rank != 2) {
          fprintf(stderr, "The three matrices for matrix multiplication are not
  2-dimensional.\n"); return;
  }
  */
  ...
  printf("A.strides[0] = %ld\n", A.strides[0]);
  printf("B.strides[0] = %ld\n", B.strides[0]);
  printf("C.strides[0] = %ld\n", C.strides[0]);
  ...
  polydl_lib_matmul_f32(M, N, K, 64, 56, 56,
                        &A.data[A.offset], &B.data[B.offset],
                        &C.data[C.offset]);
