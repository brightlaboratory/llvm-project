...
static void print(OpAsmPrinter &p, PolyDLGEMMOp op) {
	...
	string NVar = "%N";
	string KVar = "%K";
	p << MVar << " = constant " << op.M() << " : index\n";
	p << NVar << " = constant " << op.N() << " : index\n";
	p << KVar << " = constant " << op.K() << " : index\n";

	p << "call @polydl_matmul_f32( " << op.memrefA() << " , " << op.memrefB() << " , "
		<< op.memrefC() << "," << MVar << "," << NVar << "," << KVar << ")"
		" : (memref<*xf32>, memref<*xf32>, memref<*xf32>, index, index, index) -> ()";
}
