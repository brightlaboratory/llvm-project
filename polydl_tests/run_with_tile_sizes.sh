...
${llvm_project_path}/build/bin/mlir-opt --affine-polydl="tile-sizes=${M2_Tile} tile-sizes=${N2_Tile} tile-sizes=${K2_Tile} pMaps=0 pMaps=1 pMaps=2 cacheSizes=32768 cacheSizes=1048576 cacheSizes=40370176" --convert-linalg-to-affine-loops -affine-gemm-recognizer --lower-affine --convert-scf-to-std ${test_file_path}/test6.mlir | ${llvm_project_path}/build/bin/mlir-opt --convert-std-to-llvm | ${llvm_project_path}/build/bin/mlir-cpu-runner -O3 -e main -entry-point-result=void -shared-libs=${llvm_project_path}/build/lib/libmlir_runner_utils.so,${llvm_project_path}/build/lib/libmlir_c_runner_utils.so  &> run_output
sleep 1
